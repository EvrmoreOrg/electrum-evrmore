...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-Evrmore"
  !define PRODUCT_WEB_SITE "https://github.com/EvrmoreOrg/electrum-evrmore"
  !define PRODUCT_PUBLISHER "The Evrmore Community"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-evrmore-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "c:\electrum\electrum\gui\icons\electrum-evrmore.ico"
...
Section
  ...
  File "c:\electrum\electrum\gui\icons\electrum-evrmore.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-evrmore-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-evrmore-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-evrmore-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-${PRODUCT_VERSION}.exe" 0


  ;Links bitcoin: and lightning: URIs to Electrum
  ;WriteRegStr HKCU "Software\Classes\bitcoin" "" "URL:bitcoin Protocol"
  ;WriteRegStr HKCU "Software\Classes\bitcoin" "URL Protocol" ""
  ;WriteRegStr HKCU "Software\Classes\bitcoin" "DefaultIcon" "$\"$INSTDIR\electrum-evrmore.ico, 0$\""
  ;WriteRegStr HKCU "Software\Classes\bitcoin\shell\open\command" "" "$\"$INSTDIR\electrum-evrmore-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  ;WriteRegStr HKCU "Software\Classes\lightning" "DefaultIcon" "$\"$INSTDIR\electrum-evrmore.ico, 0$\""
  ;WriteRegStr HKCU "Software\Classes\lightning\shell\open\command" "" "$\"$INSTDIR\electrum-evrmore-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\electrum-evrmore.ico"
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\evrmore"
